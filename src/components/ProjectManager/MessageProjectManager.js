...
const MessageProjectManager = () => {
    ...
    useEffect(() => {
        if (username) {
            ...
            fetch(`https://revtaskman-bgf0cwfaaxg9b5e8.southindia-01.azurewebsites.net/projects/by-username?username=${username}`)
                .then(response => response.json())
                .then(projects => {
                    // Step 2: Fetch messages for each project
                    const projectMessagesPromises = projects.map(project => {
                        return fetch(`https://revtaskman-bgf0cwfaaxg9b5e8.southindia-01.azurewebsites.net/api/messages/getMessagesByReceiverName?receiverName=${username}&projectId=${project.projectId}`)
                            .then(response => response.json())
                            .then(messages => {
    ...
    const handleDelete = (projectName, messageId) => {
        ...
        if (confirmDelete) {
            fetch(`https://revtaskman-bgf0cwfaaxg9b5e8.southindia-01.azurewebsites.net/api/messages/delete/${messageId}`, {
                method: 'DELETE',
            })
            .then(response => {
                if (response.ok) {
