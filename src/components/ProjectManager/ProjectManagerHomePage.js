...
const ProjectManagerHomePage = () => {
    ...
    // Azure domain
    const baseURL = 'https://revtaskman-bgf0cwfaaxg9b5e8.southindia-01.azurewebsites.net';

    useEffect(() => {
        const fetchUserDetails = async () => {
            try {
                const response = await fetch(`${baseURL}/users/by-username?username=${username}`);
    ...
    return (
        <div className="pm-home-container">
            ...
            <div className="pm-actions-container">
                ...
                <button onClick={() => fetchData('/projects/by-username?username=' + username, 'projects')}>View all projects</button>
                <button onClick={() => fetchData('/users/clients-by-manager?managerName=' + username, 'clients')}>View all clients</button>
                <button onClick={() => fetchData('/users/tasks-by-manager?managerName=' + username, 'tasks')}>View all tasks</button>
            </div>
