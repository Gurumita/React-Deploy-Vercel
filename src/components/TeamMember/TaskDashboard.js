...
const TaskDashboard = () => {
    ...
    const fetchUserData = async () => {
        try {
            const userResponse = await fetch(`https://revtaskman-bgf0cwfaaxg9b5e8.southindia-01.azurewebsites.net/users/by-username?username=${username}`);
    ...
    const fetchProjects = async (username) => {
        try {
            const response = await fetch(`https://revtaskman-bgf0cwfaaxg9b5e8.southindia-01.azurewebsites.net/projects/user/${username}`);
    ...
    const fetchMilestones = async () => {
        try {
            const response = await fetch('https://revtaskman-bgf0cwfaaxg9b5e8.southindia-01.azurewebsites.net/milestones');
            const data = await response.json();
            setMilestones(data);
        } catch (error) {
    ...
    const fetchTasksForProject = async (projectId) => {
        try {
            const response = await fetch(`https://revtaskman-bgf0cwfaaxg9b5e8.southindia-01.azurewebsites.net/tasks/user/${userId}/project/${projectId}`);
            const data = await response.json();
    ...
    const fetchProjectManager = async (projectId) => {
        try {
            const response = await fetch(`https://revtaskman-bgf0cwfaaxg9b5e8.southindia-01.azurewebsites.net/projects/${projectId}/manager`);
    ...
    const handleDragEnd = async (result) => {
        ...
        try {
            const response = await fetch(`https://revtaskman-bgf0cwfaaxg9b5e8.southindia-01.azurewebsites.net/tasks/update-milestone?taskId=${taskId}&milestoneId=${destinationMilestone.milestoneId}`, {
                method: 'PUT',
                    ...
                    <div className="task-details" onClick={(e) => e.stopPropagation()}>
                        ...
                        <table className="task-details-table">
                            <tbody>
                                <tr>
                                    <th>Project Name</th>
                                    <td>{selectedTask.project.projectName}</td>
                                </tr>
                                <tr>
                                    <th>Task Name</th>
                                ...
                                <tr>
                                    <th>Task Details</th>
                                    <td>{selectedTask.taskDetails}</td>
                                </tr>
                                <tr>
                                    <th>Milestone Name</th>
                                    <td>{selectedTask.milestone.milestoneName}</td>
                                </tr>
                                <tr>
                                    <th>Milestone Description</th>
                                    <td>{selectedTask.milestone.milestoneDescription}</td>
                                </tr>
                                <tr>
                                    <th>Start Date</th>
                                    <td>{new Date(selectedTask.startDate).toLocaleDateString()}</td>
                                </tr>
                                <tr>
                                    <th>Due Date</th>
                                    <td>{new Date(selectedTask.dueDate).toLocaleDateString()}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
